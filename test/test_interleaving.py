        ...
        if do_simt:
            ...
            with cm:
                for idx_rep in range(num_nodes_repetition):
                    constructor.capture_library_call_begin()
                    # SIMT
                    c2 = triton_utils.run_matmul(A2s[idx_rep], B2s[idx_rep])
                    C2s.append(c2)
                    constructor.capture_library_call_end()
...
if __name__ == "__main__":
    print("Triton SIMT in parallel to Torch TensorOp. Repeat with same data")
    test_triton_simt_and_torch_tensorop_interleave()
    print("Cutlass SIMT in parallel to TensorOp. Repeat with same data")
    test_cutlass_simt_and_tensorop_interleave()
    print(
        "Triton SIMT in parallel Cutlass to TensorOp. Repeat with differnet"
        " data"
    )

...
struct SpmmKernel2 {
  ...
  static __device__ __forceinline__ void KernelFn(
    ...
    SparseTile sparse_tile_loader(n, row_offset, threadIdx.x,
                                  values + blockIdx.z * nnz, column_indices,
...
__global__ void __launch_bounds__(Config::kThreadsPerBlock)
    Kernel2(int m, int k, int n, const int* __restrict__ row_indices,
            const typename Config::ScalarValue* __restrict__ values,
            const int* __restrict__ row_offsets,
            const typename Config::ScalarIndex* __restrict__ column_indices,
            const typename Config::ScalarValue* __restrict__ dense_matrix,
...
cudaError_t CudaSpmmBiasRelu2(
  ...
  if ((n % 4) == 0) {
    if (n == 8) {
      ...
      return CudaSpmmEx2<Config>(m, k, n, nonzeros, row_indices, values,
                                 row_offsets, column_indices, dense_matrix,
                                 bias, output_matrix, stream, batch_size);
    } else if (n < 8) {
    ...
    return CudaSpmmEx2<Config>(m, k, n, nonzeros, row_indices, values,
                               ...
                               output_matrix, stream, batch_size);
  } else {
    // Scalar kernel.
...
cudaError_t CudaSpmm2(int m, int k, int n, int nonzeros,
                      ...
                      int batch_size) {
  return CudaSpmmBiasRelu2(m, k, n, nonzeros, row_indices, values, row_offsets,
                           column_indices, dense_matrix,
                           /* bias = */ nullptr, output_matrix, stream,
                           batch_size);
}

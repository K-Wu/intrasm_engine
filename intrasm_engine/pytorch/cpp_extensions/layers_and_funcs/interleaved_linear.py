...
class MyInterleavedLinear(MyAutogradFunc):
    ...
    def _forward(  # single threaded operation
        ...
        # in_sparse = (
        #     (input[0 : int(i_shape[0] / 2), :]).unsqueeze(0).contiguous()
        # )
        # in_dense = input[int(i_shape[0] / 2) :, :].contiguous()

        out_dense = torch.zeros(
            input.shape[0], w_shape[1] // 2, device="cuda", dtype=torch.float32
        ).contiguous()
        # out_sparse = torch.zeros(
        #     input.shape[0], w_shape[1] // 2, device="cuda", dtype=torch.float32
        # ).contiguous()
        # out = torch.zeros(w_shape[0], 1, device="cuda", dtype=torch.float32)

        # weight_csr = weight_sparse.to_sparse_csr()
        weight_csr_t = SparseCS(weight_sparse.t(), torch.device("cuda"))
        # input kept as dense
        ...
        if constructor_enabled:
            ...
            with constructor.registeredStreams[1] as compound_stream:
                ...
                out_sparse = (
                    torch.ops.iex_ops.spmm_sputnik_reuse_weight(
                        input.t().unsqueeze(0),
                        weight_csr_t.row_indices,
                        weight_csr_t.values.flatten(),
                        weight_csr_t.row_offsets,
                        weight_csr_t.column_indices,
                        weight.shape[1] // 2,
                    )
                    .squeeze(0)
                    .t()
            ...
            with constructor.registeredStreams[0] as compound_stream:
                out = torch.cat((out_sparse, out_dense), dim=1)

        else:
            gemm_op.operation.run(gemm_args)
            # To use SpMM, we need to transpose the weight, the dense input and the final output. (We are using the WT*XT(row-major) sputnik kernel while the actual input is X(row-major)*W. )
            out_sparse = (
                torch.ops.iex_ops.spmm_sputnik_reuse_weight(
                    input.t().unsqueeze(0),
                    weight_csr_t.row_indices,

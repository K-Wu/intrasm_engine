...
# Based on https://github.com/huggingface/transformers/blob/03cc17775b961d16cc4d0d7ab0c8487120d0b708/src/transformers/models/opt/modeling_opt.py
# coding=utf-8
    ...
    def __init__(
        ...
        **kwargs,
        ...
        def _handle_deprecated_argument(
            config_arg_name, config, fn_arg_name, kwargs
        ):
            """
            ...
            if fn_arg_name in kwargs:
                logging.warning(
                    "Passing in {} to {self.__class__.__name__} is deprecated"
                    " and won't be supported from v4.38. Please set it in the"
                    " config instead"
                )
...
class OPTDecoderLayer(nn.Module):
    def __init__(self, config: OPTConfig):
        ...
        self.fc1 = nn.Linear(
            self.embed_dim, config.ffn_dim, bias=config.enable_bias
        )
        self.fc2 = nn.Linear(
            config.ffn_dim, self.embed_dim, bias=config.enable_bias
        )
...
@add_start_docstrings(
    (
        "The bare OPT Model outputting raw hidden-states without any specific"
        " head on top."
    ),
    OPT_START_DOCSTRING,
)
        ...
        if self._use_flash_attention_2:
            ...
            if attention_mask is None:
                ...
                raise ValueError(
                    ...
                    f" {attention_mask.shape[1]}, but its length should be"
                    f" {mask_seq_length} (sum of the lengths of current and"
                    " past inputs)"
                )
        ...
        if self.config.pad_token_id is None:
            ...
            if input_ids is not None:
                ...
                logger.warning(
                    ...
                    " in `inputs_embeds`. Results may be unexpected if using"

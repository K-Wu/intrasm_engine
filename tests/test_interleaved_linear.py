...
from intrasm_engine.pytorch.cpp_extensions.layers_and_funcs.interleaved_linear import MyInterleavedLinear
...
def generate_tensors (
    ...
    weights = [
        torch.randn(out_features, in_features, device = "cuda", dtype = torch.float32)
...
def test_interleaved_linear (
    in_features, out_features, repeat_times
) :
    # interleaved_linear = MyInterleavedLinear()
    
    weights_and_inputs = generate_tensors(in_features, out_features, repeat_times)
    weights, inputs = weights_and_inputs["weights"], weights_and_inputs["inputs"]
    ...
    out = MyInterleavedLinear._forward(constructor,
                                        inputs[0],
                                        weights[0],
...
if __name__ == "__main__":
    print("testing interleaved linear layer, non batched input, currently repeating ONLY ONCE")
